...
try:
    ...
    sys.exit("libcst is required: pip install -e '.[lint]'")
...
def _is_internal_import(node) -> bool:
    """
    Whether an import statement refers to laura's own modules.
    """
    ...
    if node.module is None:
        return False
    return _dotted_name(node.module).split(".")[0] == "laura"
...
class _ProtectedNameCollector(cst.CSTVisitor):
    ...
    * keyword-argument labels -- ``f(write_GPT=1)`` refers to the *callee's*
      parameter, which is unrelated to a module-level ``write_GPT``;
    * parameter names in ``def`` signatures;
    * names bound in a local (function or comprehension) scope, which shadow the
      module-level object rather than referring to it.

    Names defined at module level, and names imported into the module, are left
    unprotected so they *do* get renamed.
    """
    ...
    def visit_ImportFrom(self, node: cst.ImportFrom) -> None:
        """
        Protect names imported from outside laura.
        """
...
GENERATED = (
    ...
    "unit_tests/test_naming_compat.py",
)


def _is_generated(path: Path) -> bool:
    rel = path.resolve().as_posix()
    return any(f"/{marker}" in rel or rel.endswith(marker) for marker in GENERATED)
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        nargs="*",
        type=Path,
        default=[REPO_ROOT / "laura", REPO_ROOT / "unit_tests"],
        help="files/dirs to rewrite (default: laura/ and unit_tests/)",
    )
    ap.add_argument(
        "--docstrings",
        action="store_true",
