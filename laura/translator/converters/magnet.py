        ...
        corners[0] = np.array(
            list(
                map(
                    add,
                    np.transpose(
                        [getattr(self.physical.start, p) for p in ["x", "y", "z"]]
                    ),
                    np.dot(
                        [-self.magnetic.width * self.magnetic.length, 0, 0],
        ...
        corners[1] = np.array(
            list(
                map(
                    add,
                    np.transpose(
                        [getattr(self.physical.end, p) for p in ["x", "y", "z"]]
                    ),
                    np.dot(
                        [-self.magnetic.width * self.magnetic.length, 0, 0],
                        _rotation_matrix(theta),
...
class WigglerTranslator(BaseElementTranslator):
    """
    Translator class for converting a :class:`~laura.models.element.Wiggler` element instance into a string or
    object that can be understood by various simulation codes.
    """
...
class NonLinearLensTranslator(BaseElementTranslator):
    """
    Translator class for converting a :class:`~laura.models.element.NonLinearLens` element instance into a string or
    object that can be understood by various simulation codes.
    """
