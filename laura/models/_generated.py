...
class _AcceleratorElementBase(ConfiguredBaseModel):
    ...
    inherits_from: Optional[str] = Field(default=None, description="""If set, this element's definition is merged on top of the named element's at load time, so it need only state what differs. Populated from ``inherit`` in YAML, matching PALS. Resolution happens on the raw dict before parsing (see ``YAML_Loader.resolve_inheritance``); position, identity and topology fields are never inherited. Unrelated to ``subelement``, which is a physical part-of relationship rather than a definitional one.""", validation_alias=AliasChoices('inherits_from', 'inherit'), json_schema_extra = { "linkml_meta": {'aliases': ['inherit'], 'domain_of': ['AcceleratorElement']} })
    """If set, this element's definition is merged on top of the named element's at load time, so it need only state what differs. Populated from ``inherit`` in YAML, matching PALS. Resolution happens on the raw dict before parsing (see ``YAML_Loader.resolve_inheritance``); position, identity and topology fields are never inherited. Unrelated to ``subelement``, which is a physical part-of relationship rather than a definitional one."""
