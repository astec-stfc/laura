...
laura is consumed by astec-stfc/simba, which imports laura internals directly
(``laura.translator.utils.fields``, ``laura.translator.conversion_rules``).
Every renamed public name here keeps working under its old spelling.

Warnings are ``FutureWarning``, not ``DeprecationWarning`` so that downstream
packages such as `SIMBA` also display the warnings.
"""
...
__all__ = ["deprecated_aliases", "DeprecatedMethodAliases", "LAURA_RENAMES"]

LAURA_RENAMES: dict[str, dict[str, str]] = {}
...
def deprecated_aliases(
    ...
    """
    LAURA_RENAMES[module_name] = dict(aliases)
...
class DeprecatedMethodAliases:
    ...
    set the mapping on each class::
        ...
        class AstraHeader(DeprecatedMethodAliases, BaseModel):
            _DEPRECATED_METHOD_ALIASES = {"write_ASTRA": "write_astra"}
        ...
        for legacy, current in inherited.items():
            if legacy in vars(cls):
                warnings.warn(
                    ...
                    f"which laura renamed to {current!r}. laura now calls "
