...
def _hardware_type_aliases() -> dict:
    """Map a ``hardware_type`` serialization alias to the value it stands for.
    Derived from the registry rather than hard-coded, so a new alias is picked up
    automatically.
    """
    aliases = {}
    for hardware_type, cls in ELEMENT_REGISTRY.items():
        field = cls.model_fields.get("hardware_type")
        alias = getattr(field, "alias", None)
        if alias and alias != "hardware_type":
            aliases[alias] = hardware_type
    return aliases


_HARDWARE_TYPE_ALIASES = None


def interpret_YAML_Element(elem: dict, exclude_set=None):
    global _HARDWARE_TYPE_ALIASES
    hw_type = elem.get("hardware_type")
    if not hw_type:
        if _HARDWARE_TYPE_ALIASES is None:
            _HARDWARE_TYPE_ALIASES = _hardware_type_aliases()
        for alias, aliased_type in _HARDWARE_TYPE_ALIASES.items():
            if alias in elem:
                hw_type = aliased_type
                break
    if not hw_type:
