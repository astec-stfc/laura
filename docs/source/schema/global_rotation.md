---
search:
  boost: 5.0
---

# Slot: global_rotation 


_Accumulated global rotation including parent-frame contributions._



<div data-search-exclude markdown="1">



URI: [laura:global_rotation](https://w3id.org/laura/global_rotation)
<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [PhysicalElement](PhysicalElement.md) | Physical placement data: position, rotation, length, and associated survey / ... |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [Rotation](Rotation.md) |
| Domain Of | [PhysicalElement](PhysicalElement.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |
### Slot Characteristics

| Property | Value |
| --- | --- |
| Owner | [PhysicalElement](PhysicalElement.md) |












## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/laura/schema




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | laura:global_rotation |
| native | laura:global_rotation |




## LinkML Source

<details>
```yaml
name: global_rotation
description: Accumulated global rotation including parent-frame contributions.
from_schema: https://w3id.org/laura/schema
rank: 1000
owner: PhysicalElement
domain_of:
- PhysicalElement
range: Rotation

```
</details></div>