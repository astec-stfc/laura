---
search:
  boost: 5.0
---

# Slot: transverse_gradient_x 


_Transverse field gradient in x [1/m]._



<div data-search-exclude markdown="1">



URI: [laura:transverse_gradient_x](https://w3id.org/laura/transverse_gradient_x)
<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [WigglerMagnet](WigglerMagnet.md) | Periodic wiggler/undulator field |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [Float](Float.md) |
| Domain Of | [WigglerMagnet](WigglerMagnet.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |
### Slot Characteristics

| Property | Value |
| --- | --- |
| If Absent | `float(0.0)` |
| Owner | [WigglerMagnet](WigglerMagnet.md) |












## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/laura/schema




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | laura:transverse_gradient_x |
| native | laura:transverse_gradient_x |




## LinkML Source

<details>
```yaml
name: transverse_gradient_x
description: Transverse field gradient in x [1/m].
from_schema: https://w3id.org/laura/schema
rank: 1000
ifabsent: float(0.0)
owner: Wiggler_Magnet
domain_of:
- Wiggler_Magnet
range: float

```
</details></div>