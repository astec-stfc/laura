# Slot: transverse_gradient_y 


_Transverse field gradient in y [1/m]._



<div data-search-exclude markdown="1">



URI: [laura:transverse_gradient_y](https://w3id.org/laura/transverse_gradient_y)
<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [WigglerMagnet](WigglerMagnet.md) | Periodic wiggler/undulator field |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [Double](Double.md) |
| Domain Of | [WigglerMagnet](WigglerMagnet.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |
### Slot Characteristics

| Property | Value |
| --- | --- |
| If Absent | `float(0.0)` |
| Owner | [WigglerMagnet](WigglerMagnet.md) |












## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/laura/schema




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | laura:transverse_gradient_y |
| native | laura:transverse_gradient_y |




## LinkML Source

<details>
```yaml
name: transverse_gradient_y
description: Transverse field gradient in y [1/m].
from_schema: https://w3id.org/laura/schema
rank: 1000
ifabsent: float(0.0)
owner: Wiggler_Magnet
domain_of:
- Wiggler_Magnet
range: double

```
</details></div>