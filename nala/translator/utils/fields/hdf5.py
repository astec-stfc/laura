...
def read_HDF5_field_file(self, filename: str) -> str:
    """
    Read an HDF5 field file and convert it into a :class:`nala.translator.utils.fields.field` object
    ...
    ------
    KeyError:
        if the `cavity_type` attribute is not in :attr:`~nala.translator.utils.fields.allowed_cavities
    KeyError:
        if `cavity_type==TravellingWave` and all the required attributes are not defined in the file;
         see :attr:`~nala.translator.utils.fields.tw_required_attrs
    Warning:
...
def write_HDF5_field_file(self):
    """
    Write the :class:`nala.translator.utils.fields.field` object to an HDF5 file.
